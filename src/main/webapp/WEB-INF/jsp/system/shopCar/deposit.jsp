...
<html>
	...
	<head>
		...
		<script>
			var hLCart = ${hLCartDPMath},
				bLCart = ${bLCartDPMath},
				gLCart = ${gLCartDPMath};
			var deposit_TF = true;	
				//为空时 报错！
			console.log(${bLCartDPMath})
		</script>
	</head>
	...
	<body id="carBox-body">
		...
		<div id="depositBox" class="carBox">
			<div id="good-category">
				<a href="<%=basePath%>cartPage/getCartFPGoods">全款商品</a>
				<a href="javascript:;" class="active">定金商品</a>
			</div>
			<div  class="car-head">
				<div class="col col0">
			...
			<div class="goods" v-cloak v-if="hLCart.length">
				<ul class="goodsBox" id="German-box">
					<li v-for="(good,index) in hLCart" class="gooditm" :class="{'no-gooditm' : good.goodsStock == 0 || good.goodsState != 4,'stockOut' : good.goodsStock > 0 && good.goodsStock < good.buyNum}" cartID="${good.cartID}" goodsID="${good.goodsID}" >
						...
						<div class="col col7">{{good.uFMoney.toFixed(2)}}</div>
						<div class="col col7">{{good.deposit}}%</div>
						<div class="col col5">
							{{good.uDMoney.toFixed(2)}}
							<div class="tax">
								<div>
									<em>
										<a href="javascript:;">?</a>
		...
		<!--结算-->
		<form method="post" action="../cartSettle/goDSettleFC" id="goFSettleFC" name="goFSettleFC" class="form-inline">
			<!--保税仓-->
