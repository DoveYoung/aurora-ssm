...
<div class="carBox">
	...
	<c:if test="${hCartFPMath.size() > 0}">
		...
		<div class="goods">
			<ul class="goodsBox" id="German-box">
				<c:forEach items="${hCartFPMath}" var="good">
					<li class="gooditm">
						...
						<div class="col col6 total-list">${good.getUMoney()}</div>
					</li>
				</c:forEach>
			</ul>
		</div>
	...
	<c:if test="${bCartFPMath.size() > 0}">
		...
		<div class="goods">
			<ul class="goodsBox" id="wareHouse-box">
				<c:forEach items="${bCartFPMath}" var="good">
					<li class="gooditm">
						<div class="hidden">${good.cartID}</div>
						<div class="col col2">
							<a href="javascript:;" class="goodImg">
	...
	<!--国内现货-->
	<c:if test="${gCartFPMath.size() > 0}">
		<div id="theSpot" class="mail">
			<i>国内现货</i>
		</div>
		<div class="goods">
			<ul class="goodsBox" id="theSpot-box">
				<c:forEach items="${gCartFPMath}" var="good">
					<li class="gooditm">
						<div class="hidden">${good.cartID}</div>
						<div class="col col2">
							<a href="javascript:;" class="goodImg">
...
<div class="payBox">
	...
	<div class="payBox-r">
		<div class="list">
			<span>商品价格总计：</span>
			<i id="payment">￥${tMoney}</i>
		</div>
		<div class="list">
			<span>运费：</span>
			<i id="postage">￥${tPostage}</i>
		</div>
		<div class="list">
			<span>应付金额总计：</span>
			<i class="all" id="tPayment">￥${pMoney}</i>
		</div>
		<div class="btnBox">
...
<!--提交订单-->
<form method="get" action="goFPayFC" id="goFPayFC" name="goFPayFC" class="form-inline">
	<!--保税仓-->
