...
/** 快钱人民币网关支付控制器（微仓订单合并付款）
 * @author BYG 2017-9-9
 * @version 1.0
 */
@Controller
@RequestMapping(value="/kqpayMWMerge")
public class KQPayMWMergeController extends BaseController {
    ...
    public ModelAndView goKQSend(String orderID, HttpServletRequest request) throws Exception {//orderID为合并支付交易IDpayOrderID
		ModelAndView mv = this.getModelAndView();
		BigDecimal shouldPay = microWarehouseServiceImpl.getMergeOrderSPByPayOID(orderID);//以元为单位
...
//		mv.addObject("orderAmount",String.valueOf(orderAmount.intValue()));
		mv.setViewName("system/pay/kqPay/kqSendMWMerge");
		return mv;
    }
		...
		if(flag){
	  		switch(Integer.parseInt(payResult))
	  		{
	  			case 10:
	  				...
	  				pduo.put("payOrderID", orderId);
					pduo.put("tradeNo", dealId);//快钱支付流水号
					pduo.put("orderState", 2);
					pduo.put("payPath", 3);//付款渠道：1支付宝；2微信；快钱
					microWarehouseServiceImpl.updateTransferMergeOrder(pduo);
  					rtnOK=1;
  					//以下是我们快钱设置的show页面，商户需要自己定义该页面。
  					rtnUrl="https://www.aurorascm.com/kqpayMWMerge/goKQShow?msg=success";
  					logger.info("kuaiqian=" + String.valueOf(rtnOK) + rtnUrl );
  					break;
	  			default:
  					rtnOK=0;
  					//以下是我们快钱设置的show页面，商户需要自己定义该页面。
  					rtnUrl="https://www.aurorascm.com/kqpayMWMerge/goKQShow?msg=false";
  					logger.info("kuaiqian=" + String.valueOf(rtnOK) + rtnUrl );
  					break;
	  		}
	  	}else{
	  		rtnOK=0;
	  		//以下是我们快钱设置的show页面，商户需要自己定义该页面。
	  		rtnUrl="https://www.aurorascm.com/kqpayMWMerge/goKQShow?msg=error";
	  		logger.info("kuaiqian=" + String.valueOf(rtnOK) + rtnUrl );
	  	}
