 ...
 */
@Controller
@RequestMapping(value="/kqpay")
public class KQPayCommonController extends BaseController {
    ...
    public ModelAndView goKQSend(String orderID, HttpServletRequest request) throws Exception {
		ModelAndView mv = this.getModelAndView();
		BigDecimal shouldPay = orderServiceImpl.getShouldPayByOID(orderID);//以元为单位0.00
...
//		mv.addObject("orderAmount",String.valueOf(orderAmount.intValue()));
		mv.setViewName("system/pay/kqPay/kqSend");
		return mv;
    }
		...
		if(flag){
	  		switch(Integer.parseInt(payResult))
	  		{
	  			case 10:
	 					...
	 					pduo.put("tradeNo",dealId);//微信端支付流水号
	 					...
	 					orderServiceImpl.updateOrderPayInfo(pduo);//更新商品支付信息，更新商品库存
	  					...
	  					rtnUrl="https://www.aurorascm.com/kqpay/goKQShow?msg=success";
	  					logger.info("kuaiqian=" + String.valueOf(rtnOK) + rtnUrl );
	  					break;
	  			default:
	  					rtnOK=0;
	  					//以下是我们快钱设置的show页面，商户需要自己定义该页面。
	  					rtnUrl="https://www.aurorascm.com/kqpay/goKQShow?msg=false";
	  					logger.info("kuaiqian=" + String.valueOf(rtnOK) + rtnUrl );
	  					break;
	  		}
	  	}else{
	  		rtnOK=0;
	  		//以下是我们快钱设置的show页面，商户需要自己定义该页面。
	  		rtnUrl="https://www.aurorascm.com/kqpay/goKQShow?msg=error";
	  		logger.info("kuaiqian=" + String.valueOf(rtnOK) + rtnUrl );
	  	}
