 ...
 */
@Controller
@RequestMapping(value="/alipayMerge")
public class AliPayMWMergeController extends BaseController {
	...
	public void Alipayment(HttpServletRequest request, HttpServletResponse response,
		...
		String notify_url = "https://www.aurorascm.com/alipayMerge/notifyUrl";
		// 页面跳转同步通知页面路径 需https://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
		String return_url = "https://www.aurorascm.com/alipayMerge/returnUrl";
		alipayRequest.setReturnUrl(return_url);		//设置回跳和通知地址
		alipayRequest.setNotifyUrl(notify_url);
		String out_trade_no = payOrderID;	//商户订单号，商户网站订单系统中唯一订单号，必填
...
//    		String total_amount = request.getParameter("total_amount"); //付款金额
    		...
    		if(trade_status.equals("TRADE_SUCCESS")){
				...
				pduo.put("payOrderID", out_trade_no);
				pduo.put("tradeNo", trade_no);//支付宝支付流水号
				pduo.put("orderState", 2);
				pduo.put("payPath", 1);//付款渠道：1支付宝；2微信；3银联
				try {
					microWarehouseServiceImpl.updateTransferMergeOrder(pduo);;//微仓订单支付成功后更新到普通订单中
				} catch (Exception e) {
