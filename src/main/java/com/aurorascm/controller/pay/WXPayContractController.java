 ...
 */
@Controller
@RequestMapping(value="/wxContractpay")
public class WXPayContractController extends BaseController {
	@Resource(name = "contractServiceImpl")
	private ContractService contractServiceImpl;

	@RequestMapping(value="/weixinPage", produces="application/json;charset=UTF-8")
    public ModelAndView goWXPayPage(String contractID) throws Exception {
		...
		mv.setViewName("system/pay/wxPayContract");
		return mv;
    }
	...
	public void wxPayment(HttpServletResponse response, String contractID)throws Exception{
		//获取订单应支付金额，以分为单位。
		String shouldPay = this.getShouldPay(contractID);
		//初始化微信支付客户端
        ...
        data.put("out_trade_no", contractID);//商户系统内部订单号，要求32个字符内，只能是数字、大小写字母_-|*@ ，且在同一个商户号下唯一
...
//        data.put("total_fee", shouldPay);	//实际支付金额
        ...
        data.put("notify_url", "https://www.aurorascm.com/wxContractpay/notify");	//支付成功异步通知地址
        data.put("trade_type", "NATIVE"); // trade_type=NATIVE时（即扫码支付），此参数必传。
        data.put("product_id", contractID);//此参数为二维码中包含的商品ID，商户自行定义。
	...
	@ResponseBody
	public Object query(String contractID)throws Exception{
		...
		try {
    		...
    		WXPay wxpay = new WXPay(config);
    		Map<String, String> data = new HashMap<String, String>();
    		data.put("out_trade_no", contractID);
    		Map<String, String> resp = wxpay.orderQuery(data);
 		    ...
 		    if (tradeState == "SUCCESS" || "SUCCESS".equals(tradeState)) {
 			    ...
 			    String shouldPay = this.getShouldPay(contractID);//获取合同应付金额
// 			    String shouldPay = "1";
 				if (cashFee.equals(shouldPay)) {
 					PageData pduo = new PageData();
 					pduo.put("contractID", contractID);
 					...
 					this.updateContractPayInfo(pduo);//更新商品支付信息，更新商品库存
 					result = "success";
 					msg = "支付成功";
 				} else {
 					PageData pduo = new PageData();
 					...
 					pduo.put("payPath", 2);//付款渠道：1支付宝；2微信；3银联
 					pduo.put("tradeNo", resp.get("transaction_id"));//微信端支付流水号
 					pduo.put("contractState", 11);//11实付金额与应付金额不等值
 					BigDecimal payMoney = new BigDecimal(resp.get("cash_fee"));
 					BigDecimal hundred = new BigDecimal("100");
 					payMoney = payMoney.divide(hundred);
 					DecimalFormat df = new DecimalFormat("0.00"); 
	 ...
	 * @throws Exception
	 */
	public String getShouldPay(String contractID)throws Exception{
		BigDecimal shouldPayment = contractServiceImpl.getContractMoneyByCID(contractID);
		BigDecimal hundred = new BigDecimal("100");
	 ...
	 * @throws Exception
	 */
	public String updateContractPayInfo(PageData pd)throws Exception{
		String result = "";
		try {
			contractServiceImpl.updateContractPayInfo(pd);
			result = "success";
		} catch (Exception e) {
			result = "error";
		}
