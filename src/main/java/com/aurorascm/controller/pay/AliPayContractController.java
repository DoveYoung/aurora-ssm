 ...
 */
@Controller
@RequestMapping(value="/alipayContract")
public class AliPayContractController extends BaseController {
	...
	public void Alipayment(HttpServletRequest request, HttpServletResponse response,
		...
		String notify_url = "https://www.aurorascm.com/alipayContract/notifyUrl";
		// 页面跳转同步通知页面路径 需https://格式的完整路径，不能加?id=123这类自定义参数，必须外网可以正常访问
		String return_url = "https://www.aurorascm.com/alipayContract/returnUrl";
		alipayRequest.setReturnUrl(return_url);		//设置回跳和通知地址
		alipayRequest.setNotifyUrl(notify_url);
		String out_trade_no = contractID;	//商户订单号，商户网站订单系统中唯一订单号，必填
    	...
    	if(signVerified) {
    		...
    		if(trade_status.equals("TRADE_SUCCESS")){
    			PageData pduo = new PageData();
					pduo.put("contractID", out_trade_no);
				...
				try {
					contractServiceImpl.updateContractPayInfo(pduo);//更新合同支付信息，更新商品库存
				} catch (Exception e) {
					String msg = "pay-合同：" + out_trade_no + "支付成功后订单信息更新失败";
					logger.info(msg);
					e.printStackTrace();
				}
				logger.info("success");
